...
| TC-LOGIN-003 | Verify session is created correctly | User is logged in | 1. Check if authenticated page and content are visible and accessible  | Authenticated content is visible and accesible | High |
...
| TC-LOGIN-008 | Verify URL after successful login | User is logged in | 1. Login successfuly | URL as provided | High |
...
| TC-LOGIN-013 | Verify user see elements visible only for a logged in user | 1. User is logged in 2. On the navigation menu are visible and accessible tabs "Accounts", "Billings", "Settings" | 1. Check the navigation menu is mentioned tabs are visible and accessible | Mentioned tabs are visible on the navigation bar | High |
| TC-LOGIN-014 | Verify user remains logged in after page refresh | User is logged in | 1. Refresh page 2. Check if authenticated content is visible and user is still logged in | User is logged in | Medium |
| TC-LOGIN-015 | Verify successful log out | User is logged in | 1. Successfuly log out 2. enter login page | User see login page, has no access to authenticated content and is logged out. | Medium |
